  ...
  <script>
    (() => {
      ...
      const defaults = {
        ...
        preferences: { fontSize: 18, fontFamily: "serif", lineHeight: 1.75, wordSpacing: 0, pageWidth: 980 }
      };
      ...
      const applyHighlights = () => {
        ...
        const ranges = state.highlights.map((item) => {
          ...
          return container ? textOffsetRange(container, item.start, item.end) : null;
        }).filter(Boolean);
