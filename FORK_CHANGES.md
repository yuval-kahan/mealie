...
- הורחבה תמיכת וידאו במתכונים לפורמטים נפוצים נוספים מעבר ל-`MP4/WebM/MOV`, כולל `AVI`, `MKV`, `FLV`, `WMV`, `MPEG/MPG`, `3GP`, `TS/MTS/M2TS` ועוד. פורמטים שהדפדפן לא יודע לנגן עדיין נשמרים כנכס מתכון להורדה.
...
- בדיקת יחידה ממוקדת בתוך קונטיינר ה-API למגבלת הפיקסלים ולעיבוד תמונה תקינה. נוסף גם קובץ regression test ב-`tests/unit_tests/services_tests/test_item_image_service.py` להרצה בסביבת הבדיקות המלאה של הפרויקט.
- `git diff --check`
